...
//
// D034 Amendment (2026-06-25): panel fires ONLY on explicit user request. Tests updated to
// use an explicit panel request ("run the expert panel on this") instead of implicit signals.
test('router: panel turn with an active project-master sets self_chair', () => {
  ...
  try {
    ...
    // D034: use an explicit panel request to trigger summon_panel=true.
    const s = runRouter(home, sid, 'run the expert panel on this architecture design');
