...
// Roster with one skill that has keywords matching "implement", "feature", etc.
const ROSTER_WITH_SKILL = {
  ...
  skills: {
    'superpowers:test-driven-development': {
      ...
      domains: ['tdd', 'testing'],
      keywords: ['implement', 'feature', 'bugfix', 'tdd'],
    },
  },
  tools: {},
  mcps: {},
...
// Case 1: keyword match + skill absent from prompt → nudge fires
await test('match + skill absent → nudge fires (additionalContext, exit 0, no deny)', () => {
  const payload = {
    tool_name: 'Agent',
    session_id: 'nudge-test',
    tool_input: {
      subagent_type: 'general-purpose',
      prompt: 'Implement the new feature: add a balance endpoint',
    },
  };
