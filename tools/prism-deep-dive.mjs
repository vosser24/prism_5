...
function renderMemoryMd({slug, profile}) {
  ...
  lines.push('');
  lines.push(`<!-- Injected at session start by the PRISM SessionStart hook as`);
  lines.push(`     additionalContext (first 200 lines / 25 KB). The project-master runs`);
  lines.push(`     as the SESSION-LEVEL agent (settings.json agent:), and the platform`);
  lines.push(`     memory: injection is documented only for *dispatched* subagents`);
  lines.push(`     (https://code.claude.com/docs/en/sub-agents § Enable persistent memory),`);
  lines.push(`     so PRISM loads this via the hook regardless of the platform path.`);
  ...
  lines.push('');
  lines.push('Imperatives drawn from the most important Locked adjudications:');
  lines.push('');
  lines.push('<!-- prism:standing-rules:start (AUTO-GENERATED from Locked adjudication **Rule:** lines by tools/lib/memory-heal.mjs — do not hand-edit between anchors) -->');
  lines.push('<!-- prism:standing-rules:end -->');
  lines.push('');
