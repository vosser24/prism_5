...
Record: `{health_status, checks_passed, checks_failed, checks_detail}`. `checks_detail` is per `commands/prism-health.md`'s "Recording results" section — carry it through, don't drop it. (The v2 sentinel `status` is reserved for orchestrator phase state.)
...
```json
{
  ...
  "health": {"health_status": "green", "checks_passed": 5, "checks_failed": 0, "checks_detail": []}
}
```
