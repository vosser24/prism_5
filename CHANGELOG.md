...
- **Phase 1.5 OOB evidence pipeline revived end-to-end (D040).** The out-of-band reviewer's agent resolver now reads the canonical `agent_type` field first (`agent_type → subagent_type → agent_name → agent`), fixing a name-resolution miss that was silently logging every call as `no-agent-name` and leaving the reviewer path dead. A new deterministic `SessionEnd` consumer (`hooks/prism-session-end.mjs`) drains the evidence-discipline ratchet (`~/.claude/.prism-phase-1-5-verdicts.jsonl`) every session instead of only when `/prism-clean` happens to run manually — mirrors the existing detached-spawn pattern (spawn, `detached:true`/`stdio:'ignore'`/`windowsHide:true`, `unref()`), fail-open by construction.
- **PRISM self-blindness canary in `/prism-health` (Step 6b).** `node tools/prism-telemetry-aggregate.mjs --blindness-canary` inspects the routing log, buckets by hook, and flags any hook with ≥20 logged entries whose most common action share is ≥95% — the exact failure class that let the Phase 1.5 reviewer log `no-agent-name` on effectively every call without anyone noticing. Fail-open (`status: "no-data"` on a missing/unreadable log); flagged hooks surface as a priority-1 item in Step 10.
- **Evidence-taxonomy "empty/zero/absent is a CLAIM, not evidence" rule** (`skills/master-orchestrator/references/evidence-taxonomy.md`, Correctness row). Before an absence/zero result is accepted as a finding it now requires an object-existence check (right DB/schema/table/column/name — a wrong name returns zero for everything) and a counterfactual check (what would have to be true for the number to be non-zero); an absence claim offered without both is REJECTED.
- **`capture-evidence-guard` PreToolUse hook** (`hooks/prism-capture-evidence-guard.mjs`) — denies `Edit`/`Write`/`MultiEdit` on adjudication/lesson files that assert an unverified factual claim without a citation, wired into the dispatcher `ROUTES` for `Edit`/`Write`/`MultiEdit`.
- **`dispatch-preamble` hook** (`hooks/prism-dispatch-preamble.mjs`) — composes on top of `anti-nesting-inject`'s footer to append the "write output to disk / no bug is a valid outcome / reproduce before you fix / report artifacts not counters" doctrine to every dispatched `Agent()` prompt; idempotent against re-dispatch and hand-pasted equivalents.
- **Advisory-emission telemetry** (`hooks/lib/prism-advisory-log.mjs`) — shared logger used by the skill-equip-nudge and skill-trigger-guard advisories to record `*_advisory` / `*_parse_error` events to the routing log, giving the blindness canary and future tuning passes a real signal for these hooks.
- **`/prism-clean` worktree-aware Step 1 (D041).** A missing `.claude/.prism-state.json` no longer triggers a blanket STOP → "run `/prism-bootstrap`". `git rev-parse --git-common-dir` now distinguishes a linked worktree (state is gitignored and legitimately absent there) from a genuinely un-bootstrapped project; in a worktree, capture proceeds with a bounded git-stats fallback and only the state-derived `--slug` pointer-append calls are skipped.

### Fixed

- **`prism-skill-equip-nudge` matcher precision.** Raised the match floor, fixed the domain-tag-hit tiebreak (a domain-tagged equal-score skill now ranks before a keyword-only one), de-duplicated weight-1 keyword-only hits from weight-2 domain-tagged hits, and added honest silence (a generic single-word/stopword-only match no longer surfaces a skill). Regression-tested against the live misfire (`node` alone no longer surfaces a skill) plus 13 other precision cases.
- **`prism-skill-trigger-guard` markdown-pipe parse bug.** The trigger table parser now fails loudly on a malformed row (logs a `skill_trigger_parse_error` event, stderr, non-silent) instead of swallowing it, while still loading and firing every well-formed sibling row in the same file (per-row failure, not whole-file).
- **`tools/install-manifest.json` was missing 3 shipped hook files** (`hooks/prism-dispatch-preamble.mjs`, `hooks/prism-capture-evidence-guard.mjs`, `hooks/lib/prism-advisory-log.mjs`) — added, plus a new static completeness guard (`tests/v3/install-manifest-completeness.test.mjs`) that walks the transitive local-import closure of every manifested hook and fails if a dispatcher-referenced or imported file is absent from the manifest.
- **Normalized legacy `memory: true` → `memory: project`** on 4 bundled agents (agent-factory, claude-master, master-orchestrator, prism-updater) — native enum; clears the `memory-field-normalize` red test.

### Changed

- **`hooks/prism-pretooluse-dispatcher.mjs` ROUTES ordering** — `prism-dispatch-preamble.mjs` now runs immediately before `prism-anti-nesting-inject.mjs` on the `Agent` route (both hooks rewrite `tool_input.prompt`; the dispatcher's `updatedInput` merge is first-writer-wins per key, and dispatch-preamble composes on top of anti-nesting's footer by calling its exported `run()` directly, so it must be the later write to win the merge — reordering these two would silently drop one hook's footer from every dispatched prompt).

## [6.2.0] - 2026-07-13

### Added

- **MEMORY.md recall self-heal on SessionStart.** Decision/lesson pointers in `.claude/agents/MEMORY.md` are now auto-upserted, and the "Standing rules" block is auto-generated from Locked adjudications' `**Rule:**` lines (capped, newest-first) instead of requiring manual curation.
- **Freshness-sweep M1 check** — a backstop that catches memory-recall drift the self-heal above misses.
- **Cross-session task persistence + resume recall.** Session-end now snapshots open tasks to a `.tasks.json` sidecar plus a project pointer; session-start injects a `<TASK-RECALL>` directive so open tasks are re-hydrated at the start of the next session.
- **Per-task step-by-step handoff format in `/prism-clean`** — plans now require an explicit step-by-step breakdown per task rather than a single summary line.
- **Anti-brevity task-description advisory** — nudges against under-specified, too-terse task descriptions.

### Fixed

- **fix(dispatch): nested-dispatch depth hardening (D039)** — mechanical anti-over-delegation prompt injection (hooks/prism-anti-nesting-inject.mjs), nested-`claude -p` guard, opt-in dispatch-signal diagnostic. Global-state hard-deny ruled infeasible under agent-teams.
- **Fork-aware anti-nesting injection wording** — forks are no longer told "you are a dispatched subagent"; the injected text now distinguishes a fork (same-model continuation) from an actual dispatched subagent.
- **Flaky `verify-nested-dispatch-fix.mjs` test-isolation** — the dispatch-dedup ledger was self-colliding across test runs, producing intermittent false failures; tests now isolate their ledger state.
- **anti-nesting-inject `updatedInput` now preserves `subagent_type`/`description`** (previously returned only `prompt`, which the runtime schema-rejected, breaking every Agent dispatch once installed).

### Changed

- **`/prism-clean` Step 4b handoff template** updated to the new per-task step-by-step format.
- **`capture-conventions.md` Standing-rules mechanism is now real** — previously documented as an aspiration, it is now auto-generated (see Added, above) rather than manually maintained.

## [6.1.0] - 2026-07-06

Session-orchestration and telemetry release for the project-master surface, plus a batch of guard/nudge correctness fixes. All items shipped and tested this session.

### Added

- **Always-on capability catalog at session start (D037).** A deterministic capability catalog (`hooks/lib/prism-capability-catalog.mjs`) is injected at session start so the project-master knows its available agents/skills/tools without a discovery round-trip.
- **Live-agents ledger + SendMessage support for the project-master.** `SubagentStart`/`SubagentStop` now maintain a session sidecar of live agents (`hooks/prism-subagent-start.mjs`, `hooks/prism-live-agents-summary.mjs`, `hooks/lib/prism-live-agents.mjs`), with an anti-confabulation rule so the master reports only agents that actually ran.
- **Tier-override telemetry (`override_rate`).** Tier overrides are now recorded so the classifier's real-world override rate is observable.
- **Compose-first guard on the agent-factory (`PRISM_COMPOSE_GUARD`).** Nudges toward composing existing Tier-1 tools before spawning a new specialist.

### Fixed

- **tier→opus escalation for complex prompts decoupled from panel-firing (D038).** D034 (explicit-only panel) had inadvertently coupled opus-tier escalation to panel-firing, causing sonnet over-selection on genuinely complex prompts. Escalation and panel-firing are now independent.
- **dispatch-guard false-positive** that blocked read-only tools for dispatched subagents.
- **F9 skill-equip nudge noise** — now word-boundary matched and capped to the top 3 suggestions.
- **`resolveAgentFile` relative-path orphan false-positive** and **dangling-skill-path detection**.
- **Removed 5 harmful output-length caps** that were truncating legitimate output.
- **RECALL-GATE on session-start memory injection** — gates MEMORY injection so it fires only when relevant.
- **Normalized legacy `memory: true` → `memory: project`** in agent frontmatter.
- **CLAUDE.md rule-1 stale classifier description** corrected to match the current classifier.

## [6.0.3] - 2026-06-26

**fix(roster): SubagentStop hook now records usage for dispatched specialists.** `hooks/prism-subagent-stop.mjs` read `input.agent_name || input.agent` to identify the finishing subagent, but dispatched specialists (via `Agent({subagent_type})`) surface their identity in `agent_type`/`subagent_type` — both source fields were absent, so the roster-increment block never ran and every agent stayed at `total_tasks_completed: 0`, leaving staleness/retirement logic blind. Now resolves the name in priority order `agent_type → subagent_type → agent_name → agent`, strips a leading `@`, and falls back to a case-insensitive roster-key match; fail-open (never throws in the hook). +4 test cases in `test-subagent-stop-precheck.mjs`.

## [6.0.2] - 2026-06-25

**fix(routing/cost): correct 3x-overstated Opus pricing + refresh model matrix.** `model-matrix.md` and the `prism-subagent-stop.mjs` spend-ledger PRICE table listed Opus at $15/$75 per MTok; actual is $5/$25 — the overstatement biased every cheapest-viable routing decision and inflated the spend ledger. Default Opus set to `claude-opus-4-8`; Fable 5 added as the optional top tier; Opus 4.1 marked deprecated. Tools/MCP registries refreshed; audit baseline seeded. No hook control-flow change beyond the PRICE constants.

## [6.0.1] - 2026-06-25

**fix(health): /prism-health reported a stale PRISM version.** `commands/prism-health.md` is LLM-interpreted and had no deterministic version source, so it read `skills/prism-plan/references/update-log.json` — which had been frozen at `5.7.0` across the 5.8→5.12→6.0 releases (it is not auto-derived). A fresh 6.0.0 install therefore self-reported "5.7.0". Fix: prism-health.md now resolves the version deterministically from `~/.claude/.prism-version` (installed marker) → `tools/install-manifest.json` `prism_version` (shipped) fallback, and is explicitly forbidden from reading update-log.json for version. `update-log.json` reconciled to current. A release-checklist bullet was added to both release smoke runbooks so the file is bumped going forward. No functional hook change.

## [6.0.0] - 2026-06-24

**fix(release): complete 6.0.0 version bump — all canonical version sources aligned.** The 3df5fc35e bump raised `tools/install-manifest.json` `prism_version` and `tools/lib/prism-state.mjs` to 6.0.0 but missed `.claude-plugin/plugin.json` (left at 5.13.0), causing `test-version-sync.mjs` to fail. Both sources now agree at 6.0.0. No functional change.

## [5.13.0] - 2026-06-22

**feat(memory): tiered persistent-memory architecture for the session-level project-master agent (D021/D022/D023).** A four-layer recall system so a `master-<slug>` agent configured via `settings.json` `agent:` actually carries durable project knowledge across sessions.

- **MEMORY SessionStart loader (MEMORY-INJECT-001):** fixes a product-bug — the agent-definition `memory:` frontmatter injects MEMORY.md only into *dispatched subagents*, never a session-level `agent:`. A new SessionStart hook reads `.claude/agents/MEMORY.md` and injects it explicitly, so the project-master gets its always-on recency-router + standing-rules layer.
- **Knowledge-index (D021):** `tools/prism-knowledge-index.mjs` builds `.claude/references/{knowledge-index,keyword-map}.md` — the full on-demand registry of `docs/prism/` adjudications + lessons, with each file's `**Rule:**` imperative extracted.
- **Session-start delta:** an on-change-only digest (cap 3 entries) computed IN-PROCESS via `tools/lib/knowledge-delta.mjs` — no `spawnSync` on the SessionStart hot path (a prior regression; enforced by the async-audit suite).
- **Lesson-match hook (D022):** `hooks/prism-lesson-match.mjs` (UPS) matches each prompt against the precomputed keyword map and injects the matching **Rule imperative text itself** (not a file pointer) at threshold 0.15, ~60-token cap, with a TTL dedup so the same ref doesn't re-fire every turn and a zero-suppress bypass for Locked adjudications. ~1.6 ms/call.
- **Tests:** `test-prism-knowledge-index.mjs` (21), `test-lesson-match.mjs` (11); full sweep 78 state + 25 hooks green. No new `settings.json` entry required (loader added to the existing SessionStart hook; matcher added to the UPS dispatcher routes). New files registered in `install-manifest.json`.

## [5.12.1] - 2026-06-19

**fix(install): sync `prism_version` so the installer reports the right version.** v5.12.0 bumped `.claude-plugin/plugin.json` to 5.12.0 but left `tools/install-manifest.json` `prism_version` at 5.11.0. The installer banner, the `~/.claude/.prism-version` marker, and `prism-installer update` all read the **manifest** field (not plugin.json), so a fresh `install.ps1` deployed the correct v5.12.0 *files* but reported "v5.11.0" and `update` said "Already at v5.11.0; nothing to do" (live-repro 2026-06-19 on a second machine). Both version sources are now `5.12.1`. New guard `tests/v3/state/test-version-sync.mjs` asserts `plugin.json.version === install-manifest.prism_version` (+ semver shape + a CHANGELOG entry for the current version) so the two canonical version sources can never drift again. No functional change to any hook — purely release-metadata correctness.

## [5.12.0] - 2026-06-19

Two companion features for agent dispatch quality: route build-class work to the RIGHT specialist (routing guard), and keep that specialist TOP-CLASS (quality gate). Both fail-open, advisory-first, wired into existing dispatchers with NO new `settings.json` entry. **Restart required.**

**feat(quality): Agent Quality Gate — independent scorecard at agent create/upgrade.** New fail-open `PostToolUse` hook (`prism-agent-quality-gate.mjs` + pure scorer `lib/prism-agent-quality.mjs`) added to the `Write`/`Edit`/`MultiEdit` routes, firing on a write to `.claude/agents/<name>.md` — the same identity-bearing signal `prism-agent-write-register` uses. It scores the just-written agent against a domain rubric and writes a `quality-scorecard` to `~/.claude/.prism-quality/<name>.json` — **a path the agent does not control**, defeating the "factory grades its own homework" problem that let an under-equipped frontend specialist ship. Motivated by a live audit: a frontend builder was `composes_with`-only (skill mentioned, never wired), `research_tier: 3` (codebase-only) for a builder role, and missing accessibility/performance/forms depth.

- **skills-wired body-grep (the keeper):** a skill counts as equipped ONLY if it is in the agent's `skills:` frontmatter AND referenced in the body (operating protocol). A `composes_with`-only entry, or a frontmatter entry with no body invocation, FAILS — the same mention-vs-equip discipline as the F9 skill-equip nudge, applied to the agent body.
- **research-tier-by-role + domain-depth:** `role: builder` agents in a known domain must clear research tier ≤ the domain bar (1 = external/NotebookLM-grounded) and carry the domain's cross-cutting depth refs; `role: recon`/`consistency-enforcer` and unknown-domain agents get a low bar (not gated). `quality_score` is 1–5 (matches the convention already documented in `agent-factory.md:323`).
- **Rubric-only `skill-domain-map.json`** (new) holds ONLY what didn't already exist — per-domain recommended skills, research sources, tools, depth refs, and `min_research_tier_for_builder`; the skill↔domain mapping is still derivable from `roster.skills[].domains` / `domain_groups` (no second source of truth). Roster schema gains net-new `role`/`research_tier`/`quality_score`/`skills_wired`/`last_enriched_at`/`last_synced_at` (reuses existing `owned_skills`/`core_domains`/`notebooklm_notebook_id` — no duplicate `equipped_skills`).
- Mode `PRISM_AGENT_QUALITY=off|advisory|enforce` (default advisory). PostToolUse cannot block; enforce escalates the message and records a `not_ready` flag in the scorecard for a failing `role: builder`. `test-agent-quality-gate.mjs` (10: pre-upgrade builder FAILS / upgraded PASSES / composes_with-only fails wired-check / recon low-bar / dispatcher fires + scorecard / enforce not_ready / off / non-agent silent / fail-open).

Adaptations from the source proposal (`docs/prism/plans/2026-06-19-agent-quality-gate-v2.md`), grounded in a red-team: hook the agent-file **Write** (identity in `file_path`) not PostToolUse:Agent (which only knows the factory ran — same limitation the dedup guard documents for the SubagentStop payload); reuse `owned_skills` instead of adding `equipped_skills`; the new map holds only rubric bits; lead with **independence** (agent-factory already has tiers + a self-verify step — the gap was that it was self-graded). Mechanism B (capability-staleness in `prism-freshness-sweep`) and `/prism-roster --quality` are deferred until the gate has run advisory.

**feat(guards): specialist-routing guard — build-class domain work can no longer silently route to `general-purpose`.** New fail-open `PreToolUse`/`Agent` guard (`prism-specialist-routing-guard.mjs`, added to the dispatcher's Agent route — NO new `settings.json` entry) that catches a BUILD/EDIT-class task dispatched to a generic catch-all agent (`general-purpose`/`claude`/absent) when a matching roster specialist exists, and recommends it (advisory) or blocks the dispatch (enforce). Read-only recon to a generic agent is never touched. Motivated by a live incident: a frontend/UI build was dispatched to `general-purpose` instead of the design specialist, drifting from the design system with no durable knowledge retained.

- **Specialist source of truth = `roster.agents` matched on the EXISTING `core_domains` field** — NOT a parallel `domain_keywords` array (an optional one is honored if a future install adds it). Archived agents and agents whose `status != 'available'` are skipped; the optional per-agent `build_class:false` flag excludes read/analysis-only specialists from build recommendations.
- **Build-vs-read classifier biased toward silence** (`buildScore > readScore`, floor 1) — false-silence is safer than false-block for a guard that can deny. Confident match (≥2 distinct domain terms) is required to block in enforce mode; weak/single-term matches only ever nudge.
- **Reachable override (red-team fix).** A `PreToolUse`/Agent hook cannot see the turn prompt, so the `!gp-force:` prefix is parsed in `hooks/lib/prism-opus-classifier.mjs` and carried onto the sentinel as `force_gp` (mirrors how `!opus-force:` → `force_opus` actually works), which the guard reads. One tri-state env switch `PRISM_SPECIALIST_GUARD=off|advisory|enforce` (default **advisory**) — not the proposal's two separate vars.
- **Parity bypasses:** three-path subagent-context bypass (`parent_tool_use_id` / `CLAUDE_CODE_ENTRYPOINT=subagent` / `sentinel.dispatched`) identical to agent-model-guard / parent-dispatch-guard; non-generic `subagent_type` (specialist/fork/orchestrator) exits early.
- **Doctrine + schema:** added the **Specialist-routing rule** to the `master-orchestrator` skill; documented the guard + optional `build_class` in `roster.json` schema notes.

Adaptations from the source proposal (`2026-06-19-specialist-routing-guard.md`), grounded in a red-team of the actual codebase: dropped the redundant `domain_keywords` field (reuse `core_domains`); fixed the unimplementable `!gp-force:` override (must ride the sentinel, not the prompt); corrected the deny convention (deny-JSON, the dispatcher emits exit 2 — `additionalContext` is the advisory channel); added the missing subagent-context bypass; collapsed two env vars into one tri-state; dropped the moot "order guards in the dispatcher" note (Agent-route guards run in parallel via `Promise.all`, not sequentially). `test-specialist-routing-guard.mjs` (12: the 7 spec cases + force_gp / archived / subagent-bypass / advisory-vs-enforce / build_class:false). Full PreToolUse regression green (golden 36/36, dispatcher 10/10, skill-equip 3/3, model-guard 6/6, router-e2e 11/11). **Restart required** — the dispatcher imports guard modules at session start.

## [5.11.0] - 2026-06-19

**feat(guards): anti-zombie dispatch guards — Bash hang-guard + Agent dispatch-dedup.** Two new fail-open `PreToolUse` guards defend against the two "zombie agent" failure modes. Root cause was NOT OS process leaks (a one-shot snapshot found exactly one `:8511` listener, no node/chroma pileup) but (a) tight state-file poll loops triggering WinError-5 kill-storms, and (b) in-process Claude Code subagents stalled on a hung `Bash` call — these have NO OS PID, so the fix is at the Bash/Agent layer, not the OS layer.

- **`prism-bash-hang-guard.mjs` (PreToolUse:Bash)** blocks a NARROW, provably-hang set: infinite loops (`while true`/`while :`/`until false`), poll-sleep loops, tight state-file/`*.lock` polls (the WinError-5 root cause), unbounded follows (`tail -f`, `watch`, `Get-Content -Wait`), long foreground `sleep` (>10s), and mass-kill storms (`taskkill /IM`/`/FI`/`*`, `pkill`, `killall`, `Stop-Process -Name`). Quoted segments + heredoc bodies are stripped first so a token inside a string can't false-trip; `run_in_background:true` exempts the loop/follow family (servers, log streaming) while mass-kills and state-polls still fire. Single-PID kills, bounded loops, short sleeps, and bounded `tail -n` all pass. `test-bash-hang-guard.mjs` (38).
- **`prism-dispatch-dedup-guard.mjs` (PreToolUse:Agent + SubagentStop)** blocks re-dispatching an EXACT-signature subagent task (subagent_type + description + first 400 chars of prompt) while an identical one is still in-flight (the double-burn pattern). Clears at `SubagentStop` (FIFO-by-type — the stop payload lacks the dispatch signature); a 12-min TTL backstops a missed clear so it can never permanently wedge dispatch. `test-dispatch-dedup-guard.mjs` (17).

Both compose via the existing single-dispatcher with NO new `settings.json` entry — hang-guard added to the Bash route, dedup to the Agent route + the `SubagentStop` set (most-restrictive-wins merge). Env switches: `PRISM_BASH_HANG_GUARD` / `PRISM_DISPATCH_DEDUP` (`hard|soft|off`), `PRISM_DISPATCH_DEDUP_TTL_MS` (clamped 30s–1h). Adaptations from the source proposal: a latent false-negative was fixed (`Get-Content -Wait` is almost always a quoted `pwsh -c` arg, so it's now `scanRaw` like `taskkill /FI`); the proposal's `prism-overlap-guard` references were dropped (that file does not exist in this repo) and the wiring/manifest adapted accordingly. Layer C (external liveness watchdog) is deferred per the proposal. Full sweep / golden 36/36 / audit 38/38 / wiring-guard 62/62 / manifest-coverage green. **Restart required** — the dispatcher imports guard modules at session start.
...
- **feat(tasks): adopt the Task API as the live-checklist surface (WS-1).** Tasks (`TaskCreate`/`TaskUpdate`/`TaskList`/`TaskGet`) are the default checklist surface since Claude Code v2.1.142, replacing `TodoWrite`; `CLAUDE_CODE_ENABLE_TASKS=0` is a backward-compat shim that turns Tasks OFF. PRISM now ships NO such key (locked by a guard test), so the interactive checklist renders zero-config. Added the four Task tools to every restricted-allowlist shipped agent (`master-orchestrator` was missing `TaskGet`; `agent-factory`, `claude-master`, `prism-updater` had none) and to the agent-factory template so future agents inherit the checklist. `test-prism-task-api-adoption.mjs` (22 cases). Documented the TTY-gate (Tasks no-op when `stdout.isTTY===false`, e.g. headless `claude -p`) and the narrow Task-tool hook-bypass risk.
- **feat(router): tier-router ambiguity floor (WS-2, [[D017]] deferred option).** A **score-0** prompt that keyword-floor would route to haiku but is **imperative** (leading action verb: "execute it", "fix all") and/or **references a file** (`@path` / path token) now routes to **Sonnet** — eliminating the ~10 conversation-model overrides per heavy session. Read-verbs ("list", "show"), interrogatives ("what does X mean"), and approvals stay haiku; clear-complexity stays opus. Deterministic, zero added LLM cost, in `hooks/lib/prism-opus-classifier.mjs` `keywordFloor()`. `test-prism-ambiguity-floor.mjs` (10 cases); no LLM classifier and no global Sonnet floor were added — D017's Locked decision stands (adjudication updated).
- **test(guard): "deployed ≠ wired ≠ works" regression guard (WS-3).** Two static guards lock the v5.9.4 integration regressions: (1) **wiring** — each `prism-*-dispatcher` must reference the leaf hooks shipped for its event (locks `sessionend→acl-sessionend`, `session-start→acl-notify`; fails on a dropped or dangling leaf); (2) **headless args** — the `claude` spawn in `prism-acl-worker.mjs` must pass `--settings {"disableAllHooks":true}` and must NOT pass `--bare` (F.7), with comments that *discuss* `--bare` stripped before assertion. `test-prism-wiring-guard.mjs` (56 cases).
- **docs(doctrine): subagent verification discipline (WS-4).** Counters are not a work proxy — a *delegating* worker reports `tool_uses=1` while the real work lives in its child's separate transcript. Doctrine added to `phase-1-execution.md` (verify artifacts via `git status`/diff + path-exists + test-pass, **scoped to output-producing dispatches**, NOT an unconditional `SubagentStop` hook — performance contract preserved) and to the `/prism-bootstrap` capture-conventions template ("verify ground truth before you capture 'it works'").
- **test(install): raise the installer-test spawn timeout 30s→90s.** A clean install copies ~100+ manifest files; off the SMB-mounted dev share that legitimately takes ~34s, marginally exceeding the old 30s cap and false-reddening every install-dependent assertion (the install itself exits 0). I/O latency, not a correctness bug.

## [5.9.4] - 2026-06-17

Closes the two deploy-gating findings from the 2026-06-17 **live CLI validation** — the Autonomous Capability Loop (WS-F) was non-functional end-to-end on two independent axes, even though every component passed mechanically. Both fixed, both re-validated live (full sweep 107/0, golden 36/36, audit 38/38, ACL E2Es 6/5/6).

- **fix(acl): production factory `--bare` → `--settings '{"disableAllHooks":true}'` (F.7, the gated finding).** `hooks/prism-acl-worker.mjs` dispatched the real agent-factory with `--bare`, which strips OAuth credential loading → every headless create failed with `"Not logged in · Please run /login"` and wrote nothing (reproduced live: `claude --bare` → not-logged-in at cost 0; without it → authenticates). `--bare` was reaching for a hook-free sandbox (so PRISM's own PreToolUse guards don't block the factory's `Write`/`Bash`), but it over-stripped. Replacing it with `--settings '{"disableAllHooks":true}'` disables hooks **without** touching auth. Validated end-to-end: agent-factory authenticated, ran, and wrote a capability `.md` with valid frontmatter to the staging dir ($0.03, 2 turns). `acl-production-factory.test.mjs` now asserts `--bare` absent + `--settings disableAllHooks` present (24 cases).
- **fix(acl): wire the ACL into its session-boundary events (0.4).** `prism-acl-sessionend.mjs` and `prism-acl-notify.mjs` were deployed but **never wired** — `prism-sessionend-dispatcher.mjs` only fanned out to the clean/git nudges, and `prism-session-start.mjs` never read the digest. So the loop never auto-fired on session end and `PRISM learned:` never surfaced at session start. Both hooks refactored to export `run(payload)` / `getNotice(home)` (keeping direct-script execution via a self-exec guard) and folded into the existing dispatchers — one node process per event, no new `settings.json` entry (preserves the "exactly ONE dispatcher per event" invariant). Integration-verified: SessionEnd dispatcher now writes the ACL spawn-marker + debounce stamp; SessionStart now surfaces `PRISM learned: …` and rotates the digest. Off-switch: `PRISM_DISABLE_ACL_NOTIFY=1`.
