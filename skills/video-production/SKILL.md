...
cat ~/.claude/skills/prism-plan/references/roster.json | grep -A5 "viral"
```
...
For CLI tools (like PRISM itself), app expert is less useful — Playwright
...
echo "Ask for help, and PRISM creates a specialist..." > scripts/scene-3-vo.txt
...
REQUIRED (brand core):
  1. Brand name? (e.g., "PRISM", "MyProduct")
...
```typescript
// src/brand.ts — AUTO-GENERATED by PRISM video-production skill
...
### PRISM Brand Preset (example)

If the user says "use PRISM brand" or this is an PRISM marketing video:

```typescript
// Pre-configured PRISM brand — can be used as-is or adapted
export const BRAND = {
  name: 'PRISM',
  colors: {
    primary: '#1B3A5C',      // navy
...
```typescript
// Top of src/PrismViralPromo.tsx
...
Example prompts (proven to work — from PRISM first video):
...
```
STEP 1: PRISM generates shot list from spec
...
STEP 2: PRISM determines capture method
