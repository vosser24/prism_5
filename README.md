...
| `/prism-deep-dive` + `master-<slug>` (v4.0) | ✅ Works | **Default-on** in `/prism-bootstrap` (v5.1; `--no-master` to opt out), or run `/prism-deep-dive` directly |
...
- 20 slash commands. Entry points by workflow:
  ...
  - **Project-master (v4.0; default-on since v5.1):** `/prism-deep-dive` (manual refresh/upgrade) — `/prism-bootstrap` auto-creates it
